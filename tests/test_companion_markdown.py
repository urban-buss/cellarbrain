...
class TestMakeSlug:
    def test_simple(self):
        assert _make_slug("Château Margaux", "Grand Vin") == "chateau-margaux-grand-vin"

    def test_accents_stripped(self):
        assert _make_slug("Château Phélan Ségur", None) == "chateau-phelan-segur"

    def test_none_winery(self):
        assert _make_slug(None, "Test Wine") == "test-wine"

    def test_both_none(self):
        assert _make_slug(None, None) == ""

    def test_max_length(self):
        slug = _make_slug("Very Long Winery Name", "Very Long Wine Name", slug_max_length=20)
        assert len(slug) <= 20
...
class TestRenderPriceTrackerSection:
    def test_no_data(self, tmp_path):
        ...
        writer.write_parquet("tracked_wine", [
            {"tracked_wine_id": 90_001, "winery_id": 1, "wine_name": "Test",
             "category": "red", "appellation_id": None,
             "dossier_path": "tracked/90001-test.md",
             "is_deleted": False, "etl_run_id": 1, "updated_at": now},
        ], tmp_path)
    ...
    def test_with_data(self, tmp_path):
        ...
        writer.write_parquet("wine", [
            {"wine_id": 1, "wine_slug": "w-test-2020",
             ...
             "is_favorite": False, "is_wishlist": False, "tracked_wine_id": 90_001,
             "full_name": "W Test 2020", "grape_type": "varietal",
             "primary_grape": "Merlot", "grape_summary": "Merlot",
             "_raw_grapes": None,
             "dossier_path": "cellar/0001-w-test-2020.md",
             "drinking_status": "unknown", "age_years": 6, "price_tier": "unknown",
             "is_deleted": False, "etl_run_id": rid, "updated_at": now},
        ], tmp_path)
        writer.write_parquet("wine_grape", [
            {"wine_id": 1, "grape_id": 1, "percentage": 100.0, "sort_order": 1,
             "etl_run_id": rid, "updated_at": now},
        ], tmp_path)
        writer.write_parquet("bottle", [
            {"bottle_id": 1, "wine_id": 1, "status": "stored", "cellar_id": 1,
             "shelf": "A1", "bottle_number": 1, "provider_id": 1,
             "purchase_date": now.date(), "acquisition_type": "market_price",
             "original_purchase_price": Decimal("20"), "original_purchase_currency": "CHF",
             "purchase_price": Decimal("20"), "purchase_currency": "CHF",
             "purchase_comment": None, "output_date": None, "output_type": None,
             "output_comment": None, "is_onsite": True,
             "is_in_transit": False,
             "etl_run_id": rid, "updated_at": now},
        ], tmp_path)
        writer.write_parquet("cellar", [
            {"cellar_id": 1, "name": "Main", "sort_order": 1, "etl_run_id": rid, "updated_at": now},
        ], tmp_path)
        writer.write_parquet("provider", [
    ...
    def test_preserves_existing_agent_content(self):
        ...
        result = render_companion_dossier(
            tw, [], "Test", None, settings, existing_content=existing,
        )
    ...
    def test_placeholder_sections_remain_pending_on_rerender(self):
        ...
        second_render = render_companion_dossier(
            tw, [], "Test", None, settings, existing_content=first_render,
        )
        assert "agent_sections_populated: []\n" in second_render
        assert "  - producer_deep_dive\n" in second_render
...
class TestGenerateCompanionDossiers:
    ...
    def test_preserves_existing_content(self, tmp_path):
        ...
        entities = {
            ...
            "winery": [{"winery_id": 1, "name": "Château Test"}],
            "appellation": [{"appellation_id": 1, "country": "France",
                            "region": "Bordeaux", "classification": None}],
        }
        settings = Settings()
