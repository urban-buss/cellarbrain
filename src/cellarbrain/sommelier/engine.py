...
def check_availability(model_dir: str) -> str | None:
    ...
    if not Path(model_dir).exists():
        return "Sommelier model not trained. Run `cellarbrain train-model` first."
    return None
...
class SommelierEngine:
    ...
    def suggest_wines(self, food_query: str, limit: int = 10) -> list[ScoredWine]:
        ...
        return [ScoredWine(wine_id=int(wid), score=score, wine_text="") for wid, score in results]
    ...
    def suggest_foods(self, wine_id: int, limit: int = 10) -> list[ScoredFood]:
        ...
        return [
            ScoredFood(
                dish_id=did,
                dish_name=food_names.get(did, did),
                score=score,
                food_text="",
            )
    ...
    def _get_wine_text(self, wine_id: int) -> str:
        ...
        return build_wine_text(
            ...
            region=row[2],
            grape_summary=row[3],
            category=normalise_category(row[4]),
        )
    ...
    def _get_food_names(self) -> dict[str, str]:
        ...
        return {table.column("dish_id")[i].as_py(): table.column("dish_name")[i].as_py() for i in range(table.num_rows)}
